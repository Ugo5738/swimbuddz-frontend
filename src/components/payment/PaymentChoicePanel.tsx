...
export interface PaymentChoicePanelProps {
  ...
  /** Called when the user confirms "Pay with Wallet". Custom amounts are not
   * supported via wallet today — wallet always pays the exact installment. */
  onPayWithWallet: () => Promise<void>;
  /** Called when the user chooses direct card/bank pay. Receives the chosen
   * amount in kobo so the parent can route to checkout with the override. */
