...
export function MembershipCard({
  ...
  const renderCard = (showExpand: boolean, qrSize = 80) => (
    <Card
      className={`relative overflow-hidden border-none text-white shadow-2xl ring-1 ring-white/10 ${config.glow} bg-gradient-to-br ${config.gradient}`}
    >
      {/* Background Pattern */}
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="relative flex-shrink-0">
              ...
              {/* Status indicator */}
              <div
                className="absolute -bottom-1 -right-1 h-5 w-5 rounded-full bg-emerald-400 border-2 border-white shadow-lg"
                title="Active"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <div
                className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider shadow-lg ${config.badge}`}
              >
                {config.label}
              </div>
      ...
      {mounted && isExpanded
        ? createPortal(
            <div
              ...
              onClick={(e) =>
                e.target === e.currentTarget && setIsExpanded(false)
              }
            >
