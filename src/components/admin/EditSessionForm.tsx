...
export function EditSessionForm({
        ...
        try {
            ...
            if (res.ok) {
                ...
                const mapped = configs.map((config: any) => ({
                    ...
                    departure_time: config.departure_time ? formatDateTimeLocal(config.departure_time) : formatDateTimeLocal(new Date(new Date(formData.start_time).getTime() - 2 * 60 * 60 * 1000).toISOString())
                }));
    ...
    const addAreaConfig = () => {
        setSelectedAreas([...selectedAreas, {
            ...
            cost: 1000,
            capacity: 4,
            departure_time: formatDateTimeLocal(new Date(new Date(formData.start_time).getTime() - 2 * 60 * 60 * 1000).toISOString())
        }]);
    };
