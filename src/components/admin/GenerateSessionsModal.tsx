...
interface GenerateSessionsModalProps {
    ...
    onGenerate: (templateId: string, weeks: number, skipConflicts: boolean) => Promise<any>;
}

const DAY_NAMES = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"];

export function GenerateSessionsModal({ isOpen, onClose, template, onGenerate }: GenerateSessionsModalProps) {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Generate Sessions from Template">
                    ...
                    <>
                        ...
                        <label className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                                <p className="text-sm font-medium text-slate-900">Skip conflicting dates</p>
                                <p className="text-xs text-slate-600">
                        ...
                        >
                            {result.skipped > 0 && (
                                <p className="mt-1 text-sm">
                                    Skipped {result.skipped} conflicting date{result.skipped !== 1 ? 's' : ''}
                                </p>
                            )}
                        </Alert>
                            ...
                            <div className="max-h-48 space-y-2 overflow-y-auto rounded-lg bg-yellow-50 p-3">
                                ...
                                <ul className="space-y-1 text-xs text-yellow-800">
                                    {result.conflicts.map((conflict: any, idx: number) => (
                                        <li key={idx}>
                                            {new Date(conflict.date).toLocaleDateString()} - {conflict.reason}
                                        </li>
                                    ))}
                                </ul>
                            </div>
