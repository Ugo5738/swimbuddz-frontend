...
export function MilestoneProgressModal({
  ...
  const [evidenceUrl, evidenceLoading] = useMediaUrl(
  ...
  const handleReview = async (action: "approve" | "reject") => {
    ...
    try {
      ...
      toast.success(
        action === "approve" ? "Milestone verified!" : "Re-demo requested",
      );
      onSuccess();
      onClose();
    } catch (error) {
        ...
        {isVerified && (
          <div className="mb-4 rounded-lg border border-green-200 bg-green-50 p-4">
            <div className="text-sm font-medium text-green-800">
              Verified on{" "}
              {new Date(currentProgress!.reviewed_at!).toLocaleDateString()}
            </div>
            {currentProgress?.coach_notes && (
