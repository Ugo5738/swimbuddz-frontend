...
export function EditProgramModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatedProgram = await AcademyApi.updateProgram(
        program.id,
        formData,
      );
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mb-2">
            Define the program curriculum as JSON. Example:{" "}
            {`{"week1": ["Water confidence", "Breathing"], "week2": ["Floating", "Kicking"]}`}
          </p>
          <textarea
            value={curriculumText}
            onChange={(e) => handleCurriculumChange(e.target.value)}
            rows={6}
            className={`w-full rounded-lg border px-4 py-2 font-mono text-sm focus:outline-none ${
              curriculumError
