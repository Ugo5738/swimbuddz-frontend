      ...
      if (memberResponse.ok) {
        ...
        if (protectedRoute) {
          ...
          const approvedTiers: string[] =
            ...
            member.membership.active_tiers.length > 0
              ? member.membership.active_tiers.map((t: string) =>
                  String(t).toLowerCase(),
                )
              : member.membership?.primary_tier
                ? [String(member.membership.primary_tier).toLowerCase()]
                : ["community"];
          ...
          const academyActive =
            academyApproved &&
            (academyUntilMs === null || academyUntilMs > now);
          const clubActive =
            clubApproved && clubUntilMs !== null && clubUntilMs > now;

          const effectiveTier = academyActive
            ? "academy"
            : clubActive
              ? "club"
              : "community";
