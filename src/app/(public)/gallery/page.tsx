...
export default function GalleryPage() {
  ...
  const photoCount = (album: Album) =>
    album.photo_count || album.media_count || 0;
    ...
    <div className="space-y-10 animate-in fade-in duration-300">
      ...
      <section className="space-y-4">
        ...
        <p className="text-lg text-slate-600 max-w-3xl">
          Browse photos from training sessions, community events, and academy
          programs.
        </p>
      </section>
            ...
            return (
              <Link key={album.id} href={`/gallery/${album.id}`}>
                <div className="group rounded-2xl overflow-hidden bg-white shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                  {/* Cover Image */}
                  <div
                    className={`relative aspect-[4/3] overflow-hidden bg-gradient-to-br ${albumGradients[album.album_type] || albumGradients.GENERAL}`}
                  >
                    {coverUrl ? (
                      <>
                        {/* Loading shimmer */}
                    ...
                    {/* Album type badge */}
                    <span
                      className={`absolute top-3 right-3 text-xs font-semibold px-3 py-1 rounded-full backdrop-blur-sm ${albumTypeColors[album.album_type] || albumTypeColors.GENERAL}`}
                    >
