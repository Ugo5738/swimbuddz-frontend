...
export default function CartPage() {
  const { cart, loading, updateItem, removeItem, applyDiscount, removeDiscount, isAuthenticated } =
    useStoreCart();
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      {isEmpty ? (
        <Card className="p-12 text-center">
          ...
          <p className="text-slate-500 mb-6">Browse our products and find something you'll love</p>
          <Link href="/store">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-slate-900 font-medium">
                    ₦{(cart?.subtotal_ngn ?? 0).toLocaleString()}
                  </span>
                </div>

                {cart?.member_discount_percent && cart.member_discount_percent > 0 && (
                  <div className="flex justify-between text-emerald-600">
                    <span>Member Discount ({cart.member_discount_percent}%)</span>
                    <span>Included</span>
                  </div>
                )}
                  ...
                  {cart?.discount_code ? (
                    <div className="flex justify-between text-emerald-600">
                      ...
                      <div className="flex items-center gap-2">
                        <span>-₦{(cart.discount_amount_ngn ?? 0).toLocaleString()}</span>
                        <button
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 min-w-0 px-3 py-2 text-sm border border-slate-200 rounded-lg focus:ring-2 focus:ring-cyan-400 focus:border-transparent"
                      />
                      <button
                        onClick={handleApplyDiscount}
                        disabled={!discountInput.trim() || applyingDiscount}
                        className="px-4 py-2 text-sm font-medium border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors whitespace-nowrap"
                      >
                ...
                <div className="pt-4 border-t border-slate-200 flex justify-between text-base font-semibold">
                  <span className="text-slate-900">Total</span>
                  <span className="text-cyan-600">₦{(cart?.total_ngn ?? 0).toLocaleString()}</span>
                </div>
              </div>
