            ...
            <section aria-live="polite">
                {loading ? (
                    ...
                    <Card className="p-8 text-center">
                        <p className="text-slate-600">No announcements yet. Check back soon!</p>
                    </Card>
                ) : (
                        ...
                        {announcements.map((announcement) => (
                            <Card key={announcement.id} className="space-y-3">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <p className="text-sm text-slate-600">{getSummary(announcement.body)}</p>
                                </div>
                                <Link href={`/announcements/${announcement.id}`} className="inline-flex text-sm font-semibold text-cyan-700 hover:underline">
                                    Read update &rarr;
                                </Link>
                            </Card>
                        ))}
