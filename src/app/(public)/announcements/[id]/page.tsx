...
export default function AnnouncementDetailPage() {
    ...
    const fetchComments = async () => {
        try {
            const response = await fetch(`${apiEndpoints.announcements}/${announcementId}/comments`);
            if (response.ok) {
    ...
    if (!announcement) {
        return (
            <div className="space-y-6 text-center py-12">
                ...
                <p className="text-slate-600">The announcement you&apos;re looking for doesn&apos;t exist or has been removed.</p>
    ...
    return (
        <div className="space-y-6">
            <Link href="/announcements" className="inline-flex items-center gap-2 text-sm font-semibold text-cyan-700 hover:underline">
            ...
            <header className="space-y-3">
                ...
                <h1 className="text-3xl md:text-4xl font-bold text-slate-900">{announcement.title}</h1>
            </header>

            <Card className="p-6 space-y-6">
                <p className="text-lg text-slate-700 whitespace-pre-wrap">{announcement.body}</p>
