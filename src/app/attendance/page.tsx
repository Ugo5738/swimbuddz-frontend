...
export default function AttendancePage() {
    ...
    async function fetchData() {
      try {
        ...
        if (sessionsData.length > 0) {
          const middaySession = sessionsData.find((s) =>
            s.title.toLowerCase().includes("midday"),
          );
          if (middaySession) {
  ...
  const totalFee =
    rideShareOption === "join" ? rideShareFee + poolFee : poolFee;

  // Calculate departure time based on route info offset
  const departureTime =
    selectedSession && routeInfo
      ? new Date(
          new Date(selectedSession.start_time).getTime() -
            routeInfo.departure_offset * 60 * 1000,
        )
      : null;
  ...
  if (rideSummary && rideSummary.rides.length > 0) {
    ...
    rideStatusMessage =
      "Ride #1: 0/4 passengers. Select a location to start filling.";
  }
  ...
  async function handleSubmit() {
    ...
    try {
      await apiPost(
        `/api/v1/attendance/sessions/${selectedSessionId}/attendance/public`,
        {
    ...
    <div className="mx-auto max-w-md space-y-8 p-8">
      <div className="text-center">
        ...
        <p className="text-slate-600">
          Please mark your attendance for the session.
        </p>
      </div>
                    ...
                    {area.pickup_locations.map((loc) => {
                      ...
                      if (route && selectedSession) {
                        const depTime = new Date(
                          new Date(selectedSession.start_time).getTime() -
                            route.departure_offset * 60 * 1000,
                        );
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start space-x-3">
                            ...
                            <div>
                              <span
                                className={`block text-sm font-medium ${isDisabled ? "text-slate-500" : "text-slate-900"}`}
                              >
                                {loc.name}
                              </span>
                              {loc.description && (
                              ...
                              {route && (
                                <div className="flex items-center space-x-3 text-xs text-slate-600 mt-1">
                                  ...
                                  {depTimeStr && (
                                    <span
                                      className="flex items-center font-medium text-slate-800"
