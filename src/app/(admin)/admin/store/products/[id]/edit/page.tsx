...
export default function EditProductPage() {
        ...
        const loadData = async () => {
            try {
                const [cats, prod] = await Promise.all([
                    ...
                    apiGet<Product>(`/api/v1/store/admin/products/${productId}`, { auth: true }),
                ]);
                setCategories(cats);
                setProduct(prod);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await apiPatch(`/api/v1/store/admin/products/${productId}`, payload, { auth: true });
                                ...
                                {variants.map((variant) => (
                                    <div key={variant.id} className="p-3 bg-slate-50 rounded-lg">
                                        <div className="flex justify-between items-start">
                                            ...
                                            <span className={`text-xs px-2 py-0.5 rounded ${(variant.quantity_available || 0) > 0
