...
import { Camera, ExternalLink, FolderOpen, Images, Plus, Settings, Trash2, Upload } from "lucide-react";
...
export default function AdminGalleryPage() {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            await apiDelete<void>(`/api/v1/media/albums/${albumToDelete.id}`, { auth: true });
    ...
    const photoCount = (album: Album) => album.media_count || album.photo_count || 0;
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-600 mt-1">Manage photo albums and uploads</p>
                    </div>
                </div>
                <Link
                ...
                <Card className="p-16 text-center bg-gradient-to-br from-slate-50 to-white">
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <div className="space-y-2">
                            <h3 className="text-xl font-semibold text-slate-900">No albums created yet</h3>
                            <p className="text-slate-600 max-w-sm">
                                Create your first album to start uploading photos from sessions and events.
                            </p>
                        </div>
                        <Link
                            href="/admin/gallery/create"
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {albums.map((album) => {
                        ...
                        return (
                            ...
                            >
                                {/* Cover Image */}
                                <div className={`relative aspect-[16/10] overflow-hidden bg-gradient-to-br ${albumGradients[album.album_type] || albumGradients.GENERAL}`}>
                                    {coverUrl ? (
                                        <>
                                            {/* Loading shimmer */}
                                            ...
                                            <img
                                                src={coverUrl}
                                                alt={album.title}
                                                className={`w-full h-full object-cover transition-all duration-500 group-hover:scale-105 ${loadedCovers.has(album.id) ? 'opacity-100' : 'opacity-0'
                                                    }`}
                                    ...
                                    {/* Album type badge */}
                                    <span className={`absolute top-3 left-3 text-xs font-semibold px-3 py-1 rounded-full text-white backdrop-blur-sm ${albumTypeColors[album.album_type] || albumTypeColors.GENERAL}`}>
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-sm text-slate-600">
                                This will permanently delete this album and all {photoCount(albumToDelete || {} as Album)} photos inside.
