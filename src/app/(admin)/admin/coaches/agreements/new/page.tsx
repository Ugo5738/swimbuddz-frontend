...
export default function NewAgreementVersionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!content.trim() || content.trim().length < 100) {
      setError("Agreement content must be at least 100 characters");
      return;
    }
    if (!effectiveDate) {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-600 mt-1">
            This will become the current version. All active coaches will be
            notified by email.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card className="p-6 space-y-4">
          ...
          <p className="text-sm text-slate-500">
            Write the agreement content in Markdown format. Use ## for section
            headers and - for bullet points.
          </p>
        ...
        <div className="flex items-center justify-between">
          ...
          <Button type="submit" disabled={submitting}>
            {submitting ? "Creating..." : "Create & Notify Coaches"}
          </Button>
        </div>
      </form>
    </div>
