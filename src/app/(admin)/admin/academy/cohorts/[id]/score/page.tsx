...
const SCORE_DESCRIPTIONS = [
    { score: 1, label: "Minimal", description: "Basic level, minimal complexity" },
...
export default function CohortScoringPage() {
    ...
    const [existingScore, setExistingScore] = useState<CohortComplexityScoreResponse | null>(null);
    ...
    // Form state
    const [step, setStep] = useState<"category" | "scoring" | "review">("category");
    ...
    async function loadData() {
        try {
            ...
            try {
                ...
                setDimensionScores([
                    { score: scoreData.dimension_1_score, rationale: scoreData.dimension_1_rationale || "" },
                    { score: scoreData.dimension_2_score, rationale: scoreData.dimension_2_rationale || "" },
                    { score: scoreData.dimension_3_score, rationale: scoreData.dimension_3_rationale || "" },
                    { score: scoreData.dimension_4_score, rationale: scoreData.dimension_4_rationale || "" },
                    { score: scoreData.dimension_5_score, rationale: scoreData.dimension_5_rationale || "" },
                    { score: scoreData.dimension_6_score, rationale: scoreData.dimension_6_rationale || "" },
                    { score: scoreData.dimension_7_score, rationale: scoreData.dimension_7_rationale || "" },
                ]);
                setPreviewResult({
    ...
    async function handlePreviewScore() {
        ...
        try {
            ...
            const result = await AcademyApi.previewComplexityScore(selectedCategory, scores);
...
function getCategoryDescription(category: ProgramCategory): string {
    const descriptions: Record<ProgramCategory, string> = {
        [ProgramCategory.LEARN_TO_SWIM]: "Beginner and foundational swimming programs",
        [ProgramCategory.SPECIAL_POPULATIONS]: "Programs for special needs, seniors, or therapy",
        [ProgramCategory.INSTITUTIONAL]: "School, corporate, or organizational programs",
        [ProgramCategory.COMPETITIVE_ELITE]: "Competitive training and elite athlete programs",
        [ProgramCategory.CERTIFICATIONS]: "Lifeguard, instructor, and certification courses",
        [ProgramCategory.SPECIALIZED_DISCIPLINES]: "Water polo, diving, synchronized swimming",
        [ProgramCategory.ADJACENT_SERVICES]: "Pool parties, swim camps, aqua fitness",
    };
                    ...
                    {eligibleCoaches.length === 0 ? (
                        ...
                        <div className="divide-y divide-slate-100">
                            {eligibleCoaches.map((coach) => (
                                <div key={coach.member_id} className="py-3 flex items-center justify-between">
                                    <div>
                                        <p className="font-medium text-slate-900">{coach.name}</p>
                                        <p className="text-sm text-slate-500">{coach.email}</p>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <span className={`px-2 py-1 rounded text-xs font-medium ${GRADE_COLORS[coach.grade]}`}>
