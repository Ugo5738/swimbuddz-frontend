...
export default function NewCohortPage() {
    ...
    const [step, setStep] = useState<"select-program" | "basics" | "schedule" | "sessions" | "review">("select-program");
                                    ...
                                    >
                                        <div className="flex justify-between items-start">
                                            ...
                                            <div className="text-right text-sm">
                                                <div className="font-medium text-slate-700">{program.duration_weeks} weeks</div>
                ...
                {step === "schedule" && (
                    <div className="space-y-6">
                        ...
                        <div className="rounded-lg bg-slate-50 p-4 text-sm text-slate-700">
                            <p><strong>Note:</strong> Sessions will be auto-generated based on this schedule after you create the cohort.</p>
                        </div>
                    </div>
                )}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="border rounded-lg p-4">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-sm">
                                    ...
                                    <div><span className="text-slate-500">Capacity:</span> {formData.capacity}</div>
                                    <div><span className="text-slate-500">Start:</span> {formData.start_date || "—"}</div>
                            ...
                            <div className="border rounded-lg p-4">
                                <h3 className="font-semibold text-slate-900 mb-2">Schedule ({calculateSessionCount()} sessions)</h3>
                                <div className="text-sm space-y-1">
                                    {schedule.map((item, index) => (
                                        <div key={index} className="text-slate-700">
                                            {item.day.charAt(0).toUpperCase() + item.day.slice(1)}: {item.startTime} – {item.endTime}
                                        </div>
                                    ))}
                                </div>
                            </div>
