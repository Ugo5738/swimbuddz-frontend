...
import { ClubApplication, completeObservedClubAssessment } from "@/lib/clubOnboarding";
    ...
    <div className="mx-auto max-w-6xl space-y-6 py-8">
      <header>
        ...
        <p className="mt-2 text-slate-600">
          Review the self-report, record the observed 10–15 minute assessment, and email a
          consistent outcome.
        </p>
      </header>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">{application.member_email}</p>
                  </div>
                ...
                <p className="mt-3 flex items-center gap-2 text-sm text-slate-600">
                  ...
                  {application.plan?.club_name} · {application.plan?.location}
                </p>
                <p className="mt-1 text-xs text-slate-500">
                  Community Experience:{" "}
                  {application.community_experience_selected ? "selected" : "opted out"}
                </p>
              </Card>
            </button>
          ))}
        ...
        {selected ? (
          <Card>
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                <h2 className="text-xl font-semibold text-slate-900">Observed assessment</h2>
                <p className="text-sm text-slate-600">{selected.member_name}</p>
              </div>
            ...
            <form onSubmit={save} className="space-y-5">
              <fieldset className="space-y-2">
                <legend className="text-sm font-semibold text-slate-900">Observed checks</legend>
                {observedChecks.map(([key, label]) => (
                  <label
                    key={key}
              ...
              <label className="block space-y-1 text-sm font-medium">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setForm((current) => ({
                      ...current,
                      outcome: event.target.value as typeof form.outcome,
                    }))
                  }
                  className="w-full rounded-lg border border-slate-200 px-3 py-2 font-normal"
                >
                  <option value="club_ready">Club-ready</option>
                  <option value="club_ready_modified">Club-ready — with coaching focus</option>
                  <option value="academy_first">Academy first</option>
                </select>
              </label>
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="space-y-1 text-sm font-medium">
                  ...
                  <input
                    type="number"
                    min="0"
                    value={form.nonstop_distance_m}
                    onChange={(event) =>
                      setForm((current) => ({ ...current, nonstop_distance_m: event.target.value }))
                    }
                    className="w-full rounded-lg border border-slate-200 px-3 py-2 font-normal"
                  />
                </label>
                <label className="space-y-1 text-sm font-medium">
                  Deep-water comfort
                  <input
                    value={form.deep_water_comfort}
                    onChange={(event) =>
                      setForm((current) => ({ ...current, deep_water_comfort: event.target.value }))
                    }
                    className="w-full rounded-lg border border-slate-200 px-3 py-2 font-normal"
                  />
                </label>
              ...
              {(
                [
                  ["primary_technique_focus", "Main coaching focus"],
                  ["first_club_milestone", "First goal"],
                  ["assessor_notes", "Internal coach notes"],
                ] as const
              ).map(([key, label]) => (
                <label key={key} className="block space-y-1 text-sm font-medium">
                  ...
                  {key === "assessor_notes" ? (
                    <span className="block text-xs font-normal text-slate-500">
                      Only visible to Admin/coaches; not shown to the swimmer.
                    </span>
                  ) : null}
                </label>
              ))}
              ...
              <label className="flex items-start gap-3 rounded-lg bg-cyan-50 p-3 text-sm text-cyan-900">
                <input
                  type="checkbox"
                  checked={form.send_result_email}
                  onChange={(event) =>
                    setForm((current) => ({ ...current, send_result_email: event.target.checked }))
                  }
                  className="mt-1"
                />
                <span>
                  ...
                  Send the assessment result, coaching focus and next steps to the member.
                </span>
              </label>
              <Button type="submit" disabled={saving} className="w-full">
                {saving ? "Saving..." : "Save assessment outcome"}
              </Button>
            </form>
          </Card>
        ) : (
          <Card className="flex min-h-64 items-center justify-center text-center text-slate-500">
            Select an application to record the assessment.
          </Card>
        )}
      </div>
    </div>
