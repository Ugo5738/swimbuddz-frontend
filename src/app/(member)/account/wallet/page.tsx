...
export default function WalletPage() {
  ...
  const reconcileTimeoutGuardRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  useEffect(() => {
    if (!returnReference || !returnReference.startsWith("TOP-")) return;
    if (handledReturnRef.current === returnReference) return;
    handledReturnRef.current = returnReference;

    let cancelled = false;
    const reconcile = async () => {
      setReconcilingReturn(true);
      try {
        const reconcilePromise = apiPost<TopupReconcileResponse>(
          `/api/v1/wallet/topups/reconcile/${encodeURIComponent(returnReference)}`,
          undefined,
          { auth: true },
        );
        const timeoutPromise = new Promise<never>((_, reject) => {
          setTimeout(
            () =>
              reject(
                new Error(
                  "Top-up confirmation timed out. Please refresh in a few seconds.",
                ),
              ),
            15000,
          );
        });
        const topup = await Promise.race([reconcilePromise, timeoutPromise]);

        if (cancelled) return;
        if (topup.status === "completed") {
          toast.success(
            `Top-up successful! +${topup.bubbles_amount} Bubbles added.`,
          );
    ...
    if (reconcileTimeoutGuardRef.current) {
      clearTimeout(reconcileTimeoutGuardRef.current);
    }
    reconcileTimeoutGuardRef.current = setTimeout(() => {
      if (!cancelled) {
        setReconcilingReturn(false);
        router.replace(pathname);
      }
    ...
    return () => {
      ...
      if (reconcileTimeoutGuardRef.current) {
        clearTimeout(reconcileTimeoutGuardRef.current);
        reconcileTimeoutGuardRef.current = null;
      }
    };
  }, [loadWallet, pathname, returnReference, router]);
