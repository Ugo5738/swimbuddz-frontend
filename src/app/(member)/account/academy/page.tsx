  ...
  const handleVerifyPayment = async (
    ...
    try {
      ...
      if (err.status === 404) {
        ...
        toast.error(
          "Payment still processing. Please try again in a few minutes.",
        );
      }
    } finally {
      setVerifyingId(null);
      ...
      const cohortsWithPrograms = await Promise.all(
        filteredCohorts.map(async (cohort) => {
          ...
          return {
            ...cohort,
            program,
            enrollmentStats: enrollmentStats || undefined,
          };
        }),
      );
      ...
      const detailedEnrollments = await Promise.all(
        myEnrollments.map(async (enrollment) => {
          ...
          const program = programId
            ? await AcademyApi.getProgram(programId)
            : undefined;
          const progress = await AcademyApi.getStudentProgress(enrollment.id);
          const milestones = programId
            ? await AcademyApi.listMilestones(programId)
            : [];
  ...
  useEffect(() => {
    if (returnedFromPaystack && !autoVerifyAttempted && !loading) {
      ...
      const timer = setTimeout(async () => {
        try {
          await apiPost(
            `/api/v1/payments/verify/${returnedFromPaystack}`,
            {},
            { auth: true },
          );
          toast.success("Payment verified successfully!");
          ...
          toast.info(
            "Payment is being processed. You can verify it manually below.",
          );
        }
    ...
    <div className="space-y-8">
      <header className="space-y-2">
        ...
        <p className="text-slate-600">
          Track your progress and enroll in new programs.
        </p>
      </header>
        ...
        {enrollments.length === 0 ? (
          ...
          <div className="space-y-6">
            {enrollments.map((enrollment) => {
              ...
              const showPaymentVerifyOption =
                isPendingPayment && (showVerifyHint || !returnedFromPaystack);
                    ...
                    {showApprovalBanner && (
                      <div className="bg-amber-50 border-b border-amber-200 px-6 py-3">
                        <p className="text-amber-800 text-sm flex items-center gap-2">
                          <span className="text-lg">⏳</span>
                          <span>
                            <strong>Payment received!</strong> Your enrollment
                        ...
                        <div className="bg-blue-50 border-b border-blue-200 px-6 py-3">
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            <p className="text-blue-800 text-sm flex items-center gap-2">
                              <span className="text-lg">💳</span>
                              <span>
                                <strong>Payment pending.</strong> If you've
                                already paid, click verify to confirm.
                              </span>
                            </p>
                            <Button
                              size="sm"
                    ...
                    <div className="px-6 py-4 bg-slate-50 border-b">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-sm text-slate-600">
                          {approvedCount} of {totalMilestones} milestones
                        </span>
                      </div>
                    ...
                    <div className="p-6">
                      ...
                      <div className="space-y-3">
                        {enrollment.milestones?.map((milestone) => {
                          const milestoneProgress = enrollment.progress?.find(
            ...
            {openCohorts.map((cohort) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`p-5 text-white ${isAtCapacity ? "bg-gradient-to-br from-purple-700 to-purple-900" : "bg-gradient-to-br from-slate-700 to-slate-900"}`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      {cohort.program && (
                        <span className="inline-flex items-center rounded-full bg-white/20 px-3 py-1 text-xs font-semibold">
                  ...
                  <div className="flex-1 p-5 space-y-4">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-medium text-slate-900">
                          {cohort.program?.price_amount
                            ? `₦${cohort.program.price_amount.toLocaleString()}`
                            : "Free"}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`font-medium ${isAtCapacity ? "text-red-600" : spotsRemaining <= 3 ? "text-amber-600" : "text-slate-900"}`}
                        >
                          {isAtCapacity
      ...
      <section className="grid gap-6 md:grid-cols-2">
        <Card className="p-6 bg-gradient-to-br from-purple-50 to-pink-50 border-purple-100">
          ...
          <p className="text-purple-700 text-sm mb-4">
            Need personalized attention? Book private sessions with our expert
            coaches to fast-track your progress.
          </p>
          <Button
            variant="outline"
        ...
        <Card className="p-6 bg-gradient-to-br from-amber-50 to-orange-50 border-amber-100">
          ...
          <p className="text-amber-700 text-sm mb-4">
            Weekend deep-dive sessions focusing on specific techniques like
            "Perfect Freestyle" or "Flip Turns".
          </p>
          <Button
            variant="outline"
