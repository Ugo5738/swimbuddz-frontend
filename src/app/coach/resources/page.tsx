...
export default function CoachResourcesPage() {
  ...
  const filteredResources = useMemo(() => {
    return resources.filter((resource) => {
      const cohortMatch =
        selectedCohortId === "all" || resource.cohort_id === selectedCohortId;
      const typeMatch =
    ...
    <div className="space-y-6">
      ...
      {resources.length === 0 ? (
        ...
        <Card className="p-6">
          <div className="rounded-lg border border-dashed border-slate-200 p-6 text-center">
            <BookOpen className="h-12 w-12 mx-auto text-slate-400 mb-3" />
            <p className="text-slate-600 font-medium">No matching resources</p>
            <p className="text-sm text-slate-500 mt-1">
  ...
  return (
    <Card className="p-4">
      <div className="flex items-start gap-3">
        <div
          className={`p-2 rounded-lg ${getTypeColor(resource.resource_type)}`}
        >
