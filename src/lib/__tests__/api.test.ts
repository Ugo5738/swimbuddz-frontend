...
describe('apiPost', () => {
    it('sends POST with JSON body', async () => {
        const fetchMock = vi.mocked(fetch);
        fetchMock.mockResolvedValue(mockResponse(JSON.stringify({ created: true })));
...
describe('apiPut', () => {
    it('sends PUT with JSON body', async () => {
        const fetchMock = vi.mocked(fetch);
        fetchMock.mockResolvedValue(mockResponse(JSON.stringify({ updated: true })));
...
describe('apiPatch', () => {
    it('sends PATCH with JSON body', async () => {
        const fetchMock = vi.mocked(fetch);
        fetchMock.mockResolvedValue(mockResponse(JSON.stringify({ patched: true })));
...
describe('Error handling', () => {
    it('throws with detail message from JSON error response', async () => {
        ...
        await expect(apiGet('/api/v1/members/999')).rejects.toThrow('Member not found');
    });
    ...
    it('throws with raw text when response is not JSON', async () => {
        ...
        await expect(apiGet('/api/v1/test')).rejects.toThrow('Internal Server Error');
    });

    it('throws with status code when response body is empty', async () => {
        const fetchMock = vi.mocked(fetch);
        fetchMock.mockResolvedValue(
            mockResponse('', { status: 502, ok: false }),
        );

        await expect(apiGet('/api/v1/test')).rejects.toThrow('Request failed with status 502');
    });
    ...
    it('throws for 403 Forbidden', async () => {
        ...
        await expect(apiGet('/api/v1/admin/test')).rejects.toThrow('Insufficient permissions');
    });
});
