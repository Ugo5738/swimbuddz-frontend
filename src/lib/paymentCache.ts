...
export function savePaymentIntentCache(intent: CachedPaymentIntent, memberKey?: string) {
    ...
    try {
        window.localStorage.setItem(PAYMENT_INTENT_CACHE_KEY, JSON.stringify(payload));
    } catch {
        // Ignore localStorage failures.
    }
}

export function loadPaymentIntentCache(memberKey?: string, maxAgeMs = PAYMENT_INTENT_MAX_AGE_MS): CachedPaymentIntent | null {
