...
export const CATEGORY_GROUPS: Record<
  string,
  { label: string; categories: VolunteerRoleCategory[] }
> = {
  session: {
    label: "Session Roles",
    categories: [
  ...
  community: {
    ...
    categories: ["welcome", "ride_share", "mentor"],
  },
  content: {
    label: "Content & Media",
    categories: ["media", "gallery_support"],
  },
  events: {
    label: "Events & Logistics",
    categories: ["events_logistics", "trip_planner"],
  },
...
export const VolunteersApi = {
  ...
  admin: {
    ...
    updateProfile: (memberId: string, data: Partial<VolunteerProfile>) =>
      apiPatch<VolunteerProfile>(`${VA}/profiles/${memberId}`, data, {
        auth: true,
      }),
    ...
    updateOpportunity: (id: string, data: Partial<VolunteerOpportunity>) =>
      apiPatch<VolunteerOpportunity>(`${VA}/opportunities/${id}`, data, {
        auth: true,
      }),
    ...
    listSlots: (opportunityId: string) =>
      apiGet<VolunteerSlot[]>(`${VA}/opportunities/${opportunityId}/slots`, {
        auth: true,
      }),
    ...
    markNoShow: (slotId: string) =>
      apiPost<VolunteerSlot>(`${VA}/slots/${slotId}/no-show`, undefined, {
        auth: true,
      }),
